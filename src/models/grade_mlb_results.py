...
Step 1: call grade_mlb_picks.main() to upsert per-pick grades into
        mlb_pick_grades (WIN / LOSS / VOID rows with units_result).
Step 2: aggregate those grades into mlb_pick_results (one row per
        market bucket: overall, moneyline, totals, run_line,
...
def main() -> None:
    print("=== Step 1: grade individual MLB picks ===")
    _grade_picks()
